...
const excelToJson = (function () {
  ...
  const parseSheet = (sheetData, workbook) => {
    ...
    for (let cell in sheet) {
      ...
      if (dataVariables) {
        dataVariables.forEach((dataVariable) => {
          ...
          let variableValue;
          // VT: maps keys to ref column
          dataVariableRef = `${dataVariableRef}${row}`;
      ...
      // VT: add cols as array
      if (!rowData[columnData]) rowData[columnData] = [];
      rowData[columnData].push(getSheetCellValue(sheet[cell]));
    ...
    // VT: create output object
    let result = {};
    const screen = _config?.objectScreen;
    const section = _config?.objectName;
    _config?.groupsMap?.forEach((header, i) => {
      const lang = header.toLowerCase();

      result[lang] = {};
      result[lang]["_group"] = section;
      result[lang]["_subgroup"] = screen;
      result[lang]["_lang"] = lang;
      result[lang]["_content"] = {};
